...
from app.ml_codes.schemas import CafeSWOTAnalysis, SWOTAnalysis

SWOT_ANALYSIS_TEMPLATE = Template("""\
# Role
You are a professional business analyst adept at making SWOT (Strengths, Weaknesses, Opportunities, Threats) analysis. You will make a SWOT analysis of several cafes against an opportunity analysis of an area.

# Instructions
You will be given the profiles of several cafes as an input along with a demographic analysis of an area. For each cafe, you will output a swot analysis as follows:
...
First output your thinking, then provide the final output.
...
- Focus on immediate factors and not global / macro factors like the economy, imports / exports, government regulations, etc.
...
def create_cafe_summary(generated_profile: CafeProfile) -> str:
    ...
    all_summaries.append(
        f"One sentence summary of the cafe: {profile_obj['one_sentence_summary']}."
    )
...
def generate_cafe_summary(user_query: UserQuery) -> str:
    ...
    summary = gemini_agent.run_sync(
        GENERATE_SUMMARY_TEMPLATE.render(description=cafe_description)
    ).output
    return summary
...
def preprocess_subjects(subject_list: list[CafeProfile | UserQuery]) -> list[str]:
    ...
    for subject in subject_list:
        if type(subject) is CafeProfile:
            preprocessed.append(create_cafe_summary(subject))
        else:
            preprocessed.append(generate_cafe_summary(subject))
    return preprocessed


def generate_swot_analysis(
    subject_list: list[CafeProfile | UserQuery], opportunity_summary: str
) -> list[dict[str, SWOTAnalysis]]:
    preprocessed_subjects = preprocess_subjects(subject_list)

    raw_swot_analysis = gemini_agent.run_sync(
        SWOT_ANALYSIS_TEMPLATE.render(
            cafe_profiles=preprocessed_subjects,
            opportunity_analysis=opportunity_summary,
        ),
        output_type=CafeSWOTAnalysis,
    ).output
    return raw_swot_analysis.output
